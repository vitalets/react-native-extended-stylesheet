...
describe('value', function () {
  ...
  it('should calc var', function () {
    ...
    expect(() => new Value('$xxx', 'prop', varsArr).calc()).toThrowError(
      'Unresolved variable: $xxx'
    );
  ...
  it('should not calc operations for non-operation values', function () {
    expect(new Value('flex-start', 'prop', varsArr).calc()).toEqual(
      'flex-start'
    );
  ...
  it('should apply scale', function () {
    expect(new Value('0.5rem', 'propWidth', [{ $scale: 3 }]).calc()).toEqual(
      24
    );
  ...
  it('should throw for complex operations', function () {
    expect(() => new Value('$c - $a + 1', 'prop', varsArr).calc()).toThrow(
      'Unresolved variable: $c - $a'
    );
  });
});
