...
describe('operation', function () {
  it('should detect *', function () {
    expect(operation.isOperation('10 * 20')).toEqual({
      operator: '*',
      v1: '10',
      v2: '20',
    });
    expect(operation.isOperation('$abc*100%')).toEqual({
      operator: '*',
      v1: '$abc',
      v2: '100%',
    });
  ...
  it('should throw on invalid data', function () {
    expect(() =>
      operation.exec({ operator: 'a', v1: 10, v2: 0.5 })
    ).toThrowError('Unknown operator: a');
    expect(() =>
      operation.exec({ operator: '+', v1: '10', v2: 0.5 })
    ...
    expect(() =>
      operation.exec({ operator: '+', v1: 10, v2: null })
